...
var TooltipProvider = (props) => {
  ...
  const isOpenDelayedRef = React14.useRef(true);
  const isPointerInTransitRef = React14.useRef(false);
  ...
  return (0, import_jsx_runtime11.jsx)(
    TooltipProviderContextProvider,
    {
      ...
      isOpenDelayedRef,
      delayDuration,
      onOpen: React14.useCallback(() => {
        window.clearTimeout(skipDelayTimerRef.current);
        isOpenDelayedRef.current = false;
      }, []),
      onClose: React14.useCallback(() => {
        window.clearTimeout(skipDelayTimerRef.current);
      ...
      onPointerInTransitChange: React14.useCallback((inTransit) => {
...
var Tooltip = (props) => {
  ...
  const wasOpenDelayedRef = React14.useRef(false);
  ...
  const stateAttribute = React14.useMemo(() => {
  ...
  return (0, import_jsx_runtime11.jsx)(Root22, { ...popperScope, children: (0, import_jsx_runtime11.jsx)(
    TooltipContextProvider,
    {
      ...
      onTriggerEnter: React14.useCallback(() => {
...
var TooltipTrigger = React14.forwardRef(
  (props, forwardedRef) => {
    ...
    const handlePointerUp = React14.useCallback(() => isPointerDownRef.current = false, []);
    React14.useEffect(() => {
...
var TooltipContentHoverable = React14.forwardRef((props, forwardedRef) => {
  const context = useTooltipContext(CONTENT_NAME2, props.__scopeTooltip);
  const providerContext = useTooltipProviderContext(CONTENT_NAME2, props.__scopeTooltip);
  const ref = React14.useRef(null);
  const composedRefs = useComposedRefs(forwardedRef, ref);
  const [pointerGraceArea, setPointerGraceArea] = React14.useState(null);
  const { trigger, onClose } = context;
  const content = ref.current;
  const { onPointerInTransitChange } = providerContext;
  const handleRemoveGraceArea = React14.useCallback(() => {
    setPointerGraceArea(null);
    onPointerInTransitChange(false);
  }, [onPointerInTransitChange]);
  const handleCreateGraceArea = React14.useCallback(
    (event, hoverTarget) => {
...
var TooltipContentImpl = React14.forwardRef(
  (props, forwardedRef) => {
    const {
      __scopeTooltip,
      children,
